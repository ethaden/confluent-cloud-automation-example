...
resource "confluent_service_account" "example_env_admin" {
  ...
  description  = "Service Account Example Environment Admin"
}
...
resource "local_sensitive_file" "client_config_files" {
  ...
  for_each = var.ccloud_cluster_generate_client_config_files ? {
    "admin" = confluent_api_key.example_api_key_sa_cluster_admin,
    "producer" = confluent_api_key.example_api_key_producer,
    "consumer" = confluent_api_key.example_api_key_consumer} : {}
  ...
  content = templatefile("${path.module}/templates/client.conf.tpl",
  {
    ...
    schema_registry_user = "TODO"
    schema_registry_password = "TODO"
  }
  )
